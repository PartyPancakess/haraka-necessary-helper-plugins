...
describe('lock()', () => {
    ...
    it('gets the lock lease ID', () => __awaiter(this, void 0, void 0, function* () {
        const lock = yield client.lock('resource');
        chai_1.expect(yield lock.leaseId()).to.equal(null, 'expected no lease initially');
        yield lock.acquire();
        const leaseId = yield lock.leaseId();
        chai_1.expect(leaseId).to.be.a('string');
        chai_1.expect((yield client.get('resource').exec()).kvs[0].lease).to.equal(leaseId);
        yield lock.release();
    }));
});
