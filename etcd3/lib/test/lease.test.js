...
describe('lease()', () => {
    ...
    it('attaches leases through transactions', () => __awaiter(this, void 0, void 0, function* () {
        lease = client.lease(100);
        yield lease.put('leased').value('foo');
        const result = yield client
            ...
            .commit();
        chai_1.expect(result.succeeded).to.equal(true, 'expected to have completed transaction');
        chai_1.expect((yield client.get('leased').exec()).kvs[0].lease).to.equal(yield lease.grant());
        yield lease.revoke();
        chai_1.expect(yield client.get('leased').buffer()).to.be.null;
    }));
